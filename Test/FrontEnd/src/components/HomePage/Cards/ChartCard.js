...
const ChartCard = ({sensor, showRoom}) => {
    ...
    const actual_value = sensor.value.length!==0? sensor.value.at(-1).value : '...';
    ...
    const TooltipContent = ({text, targetItem}) => {
        ...
        return (
            <div style={{zIndex:99}}>
                <Typography component="h6" variant="h6" align={"center"} style={{padding: "5px 30px 0px 30px", zIndex:99}}>
        ...
        <Paper className={classes.root}>
            {data &&
                <Chart data={data} className={classes.char}>
                    ...
                    <EventTracker/>
                    <Tooltip targetItem={target} zIndex={99} onTargetItemChange={setTarget} contentComponent={TooltipContent}/>
                </Chart>
            }
        </Paper>
    );
