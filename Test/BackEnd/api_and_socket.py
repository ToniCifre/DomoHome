...
class ApiAndSocket:
    ...
    def __init__(self, db: DBController):
        ...
        CORS(self.app, resources={r"/api/*": {
            "origins": ["http://localhost:3000", "http://192.168.3.45:3000", 'https://home.tonicifre.com']}})

    def run(self):
        self.logger_api.info('Start')
        serve(self.app, host=getenv('API_HOST'), port=int(getenv('API_PORT')))
        # self.app.run(host=getenv('API_HOST'),
        #              port=int(getenv('API_PORT')))
        # ssl_context=(Path(getenv('API_CERT_FILE')).resolve(), Path(getenv('API_KEY_FILE')).resolve()))
    ...
    def api_call_backs(self):
        ...
        def get_all_schedulers():
            return jsonify(self.db.scheduler.get_all())
        ...
        @self.app.route('/api/v1/schedulers/<scheduler_id>', methods=['DELETE'])
        @self.auth.login_required(role=0)
        def delete_scheduler(scheduler_id):
            if self.db.scheduler.delete(scheduler_id):
                return 'ok', 200
            return 'Cant remove trigger from DB.'
